...
module.exports = {
  ...
  async getAll(req,res){
    try {
      ...
      return Footer.find().exec().then((footer)=>{
        if(!footer) return res.status(401).send({success:false,message:'No footer found'})
  ...
  async getFooterById(req,res){
    try {
      ...
      const db = await connectToDatabase()
      return Footer.findById(id).exec().then((footer)=>{
        if(!footer) return res.status(401).send({success:false,message:'No footer found'})
        else return res.status(201).send({success:true,footer})
      })
    } catch (e) {
  ...
  async add(req,res){
    try {
      const db = await connectToDatabase()
      return Footer.deleteMany({}).exec().then((footer)=>{
      ...
      return Footer
            ...
            .then(footer => {
              ...
              return footer.updateOne({
                ...
                company_copyright:req.body.company_copyright || footer.company_copyright,
