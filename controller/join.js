...
module.exports = {
  async getJoinById(req,res){
    try {
      ...
      return Join.findById(id).exec().then((join)=>{
        if(!join) return res.status(401).send({success:false,message:'No join found'})
    ...
    try {
      ...
      return Join
            ...
            .then(join => {
              ...
              return join.updateOne({
                join_form_title:req.body.join_form_title || join.join_form_title,
                join_form_button:req.body.join_form_button || join.join_form_button,
