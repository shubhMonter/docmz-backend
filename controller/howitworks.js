...
module.exports = {
  async getHowitworksById(req,res){
    try {
      ...
      return Howitworks.findById(id).exec().then((howitworks)=>{
        if(!howitworks) return res.status(401).send({success:false,message:'No howitworks found'})
