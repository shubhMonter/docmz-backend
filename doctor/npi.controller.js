...
function addDoctors(req, res) {
    ...
    //Reading the File
    fs.readFile(filePathForDoctors, {
        encoding: 'utf-8'
        ...
        csvParser(csvData, {
            ...
            if (err) {
                ...
                data.map(el => {
                    ...
                    const NPIApi = `https://npiregistry.cms.hhs.gov/api/?number=` + el + `&version=2.0`
                    ...
                    request.get(NPIApi, async function (err, resp, body) {
                        ...
                        let ifExists = await Practise.findOne({ npi: doctorInfo.number });
                        ...
                        if (ifExists) {
                            ...
                            basic.organization_name = doctorInfoBasic.organization_name ? doctorInfoBasic.organization_name : "not found",
                                basic.organizational_subpart = doctorInfoBasic.organizational_subpart ? doctorInfoBasic.organizational_subpart : "not found",
                                basic.enumeration_date = doctorInfoBasic.enumeration_date ? doctorInfoBasic.enumeration_date : "not found",
                                basic.last_updated = doctorInfoBasic.last_updated ? doctorInfoBasic.last_updated : "not found",
                                basic.status = doctorInfoBasic.status ? doctorInfoBasic.status : "not found",
                                basic.credential = doctorInfoBasic.authorized_official_credential ? doctorInfoBasic.authorized_official_credential : doctorInfoBasic.credential,
                                basic.first_name = doctorInfoBasic.authorized_official_first_name ? doctorInfoBasic.authorized_official_first_name : doctorInfoBasic.first_name,
                                basic.last_name = doctorInfoBasic.authorized_official_last_name ? doctorInfoBasic.authorized_official_last_name : doctorInfoBasic.last_name,
                                basic.middle_name = doctorInfoBasic.authorized_official_middle_name ? doctorInfoBasic.authorized_official_middle_name : doctorInfoBasic.middle_name,
                                basic.telephone_number = doctorInfoBasic.authorized_official_telephone_number ? doctorInfoBasic.authorized_official_telephone_number : "not found",
                                basic.title_or_position = doctorInfoBasic.authorized_official_title_or_position ? doctorInfoBasic.authorized_official_title_or_position : "not found",
                                basic.name_prefix = doctorInfoBasic.name_prefix ? doctorInfoBasic.name_prefix : "not found",
                                basic.name_suffix = doctorInfoBasic.name_suffix ? doctorInfoBasic.name_suffix : "not found",
                                basic.sole_proprietor = doctorInfoBasic.sole_proprietor ? doctorInfoBasic.sole_proprietor : "not found",
...
function getNPI(npiNumber, response) {
    ...
    const NPIApi = `https://npiregistry.cms.hhs.gov/api/?number=` + npiNumber + `&version=2.0`
...
function getAllDoctors(req, res){
    Practise.find().populate("taxonomies").populate("address").then((data) => res.json({status: true, data})).catch((error) => res.json({status: false, error}))
}
